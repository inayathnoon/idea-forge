...
describe('Layer Dependencies', () => {
  ...
  test('no backward layer dependencies exist', () => {
    ...
    for (const file of files) {
      ...
      const imports = (content.match(/(?:import|require)\s*[\(\']([^\)\']+)/g) || [])
        .map(m => m.replace(/(?:import|require)\s*[\(\']/, ''));
    ...
    if (violations.length > 0) {
      ...
      console.error('  See ARCHITECTURE.md for layer boundaries\n');
  ...
  test('all source files belong to a recognized layer', () => {
    ...
    if (orphans.length > 0) {
      console.error('\nORPHANED FILES: Place files inside a recognized layer directory\n');
      console.error('  Recognized layers: ' + Object.keys(LAYERS).join(', '));
      orphans.forEach(f => console.error(`  ${path.relative(process.cwd(), f)}`));
    }
